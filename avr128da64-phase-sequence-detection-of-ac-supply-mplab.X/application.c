...
{	                 
 switch(sch_status)
 {
	...
	break;
 	case RMV_R_S:                                                         //remove L1 line from the 3-phase 
		sineWaveGeneration(WfClickSG_3,ZERO_FREQ,PHASE_SHIFT1);    
	break;
	case RMV_Y_S:                                                         //remove L2 line from the 3-phase
	    sineWaveGeneration(WfClickSG_2,ZERO_FREQ,PHASE_SHIFT1);
	break;
	case RMV_B_S:                                                         //remove L3 line from the 3-phase 
...
#else
	  ADC_Results_L[i] =  ADC0_GetDiffConversion(Lines[i],ADC_MUXNEG_GND_gc); // ADC0_GetConversion(Lines[i]); 
#endif	 
...
static void phaseLossDetection()
{
	uint16_t j = 0;
		...
		for (j= 0;j<=2;j++)                                                     //Loop 3 time for Three-Phases 
		{
				...
				dataStremerFrame_u.dataStreamer_st.phaseLossFlag[j] = PhaseLossFlag[j];     //Phase loss
		}
 ...
 *
 * API to decrease the amplitude of the 3 phase signals (L1 L2 L3)
 *
 * @param void
 * @return void
 ******************************************************************************/
...
/*******************************************************************************
 * static uint32_t waveform_aproxFreqcalculation(float freqency)
 *
...
static uint32_t waveformAproxFreqcalculation(float freqency)
{
	...
	calculation =(uint32_t) (freqency * ( WVVEFORM_CONSTANT / WAVEFORM_OSC_FREQ ) + 0.5);
	
	return calculation;
}

/*******************************************************************************
 * static uint16_t waveform_aproxPhasecalculation(float phase)
 *
